...
custom_stop_words <- tibble(
  word = c("legal", "agent", "pr", "intern", "social", "media", "judge", "quality")
)
...
filter_messages <- function(data, date_range, selected_agents, selected_channels, selected_risks) {
  ...
  if (!is.null(selected_agents) && length(selected_agents) > 0 && !"All" %in% selected_agents) {
    filtered <- filtered |>
      filter(agent_label %in% selected_agents | agent_role %in% selected_agents)
  }
  filtered
}
...
empty_network <- function(message, height = "560px") {
  visNetwork(
    ...
    visNodes(color = list(background = "#F1F5F9", border = "#94A3B8"),
             font  = list(color = "#475569", size = 15)) |>
...
build_alvin_keyword_network <- function(data, keywords_per_agent = 10) {
  words <- data |>
    ...
    ungroup()
  if (nrow(words) == 0) return(list(nodes = data.frame(), edges = data.frame()))
  agent_nodes <- words |>
    distinct(agent_id) |>
    transmute(
...
q1_prepare_day_data <- function(data, selected_date, selected_agents,
  ...
  selected_data |>
    ...
    filter(!is.na(q1_event_time), !is.na(q1_target), q1_target != "",
           q1_target != "Unknown", q1_period != "Unknown")
}

q1_build_centrality_network <- function(data, top_nodes = 8) {
  empty_result <- list(nodes = data.frame(), edges = data.frame(),
                       agent_ranking = data.frame(), target_ranking = data.frame())
  ...
  edge_summary <- data |>
    ...
    summarise(interactions = n(), total_risk = sum(q1_risk_weight, na.rm = TRUE),
              high_risk_messages = sum(risk_signal_group == "High Signal", na.rm = TRUE),
              .groups = "drop") |>
    mutate(from = paste0("agent::", agent_label), to = paste0("target::", q1_target))
  if (nrow(edge_summary) == 0) return(empty_result)
  ...
  edges <- edge_summary |>
    transmute(from, to, value = interactions, width = pmax(1, log1p(interactions) * 2),
              arrows = "to",
              title = paste0("<b>Interactions:</b> ", interactions,
  ...
  agent_ranking <- nodes |> filter(node_type == "Agent") |>
    transmute(Agent = label, `Subgraph centrality` = round(subgraph_centrality, 2),
              Connections = connections, Interactions = interactions) |>
...
q1_draw_centrality_network <- function(network_data, height = "600px") {
  ...
  visNetwork(network_data$nodes, network_data$edges, width = "100%", height = height) |>
    ...
    visPhysics(solver = "forceAtlas2Based",
               forceAtlas2Based = list(gravitationalConstant = -65, springLength = 115,
                                       springConstant = 0.04), stabilization = TRUE) |>
    visLayout(randomSeed = 42)
}

q1_emerging_bigrams <- function(data, afternoon_network, phrase_count = 12) {
  empty_result <- tibble(bigram = character(), Morning = integer(),
                         Afternoon = integer(), Shift = integer())
  if (nrow(data) == 0) return(empty_result)
  top_agents  <- afternoon_network$agent_ranking  |> slice_head(n = 3) |> pull(Agent)
  top_targets <- afternoon_network$target_ranking |> slice_head(n = 3) |> pull(`Channel / Recipient`)
  focused_data <- data |> filter(agent_label %in% top_agents, q1_target %in% top_targets, full_text != "")
  if (nrow(focused_data) == 0) return(empty_result)
  phrases <- focused_data |>
    select(q1_period, full_text) |>
    unnest_tokens(bigram, full_text, token = "ngrams", n = 2) |>
    separate(bigram, into = c("word1", "word2"), sep = " ", remove = FALSE) |>
    filter(!is.na(word1), !is.na(word2),
           !word1 %in% tidytext::stop_words$word, !word2 %in% tidytext::stop_words$word,
           str_detect(word1, "^[a-zA-Z]+$"), str_detect(word2, "^[a-zA-Z]+$")) |>
    count(q1_period, bigram, name = "n") |>
    pivot_wider(names_from = q1_period, values_from = n, values_fill = 0)
  if (!"Morning"   %in% names(phrases)) phrases$Morning   <- 0
  if (!"Afternoon" %in% names(phrases)) phrases$Afternoon <- 0
  phrases |> mutate(Shift = Afternoon - Morning) |>
    filter(Shift > 0) |>
...
q1_vocabulary_similarity <- function(data) {
  ...
  purrr::map_dfr(seq_len(nrow(bin_words) - 1), function(i) {
    ...
    similarity <- if (union_size == 0) 0 else length(intersect(words_a, words_b)) / union_size
    tibble(time_bin = bin_words$time_bin[[i + 1]], jaccard_similarity = similarity)
  })
}

q1_adamic_adar_timeline <- function(data, top_agents) {
  empty_result <- tibble(time_bin = as.POSIXct(character()), adamic_adar = numeric())
  if (nrow(data) == 0 || length(top_agents) < 2) return(empty_result)
  ...
  data |>
    ...
    group_modify(~ {
      ...
      if (nrow(edges) == 0 || !all(selected_actors %in% edges$agent_label))
      ...
      if (length(shared_targets) == 0) return(tibble(adamic_adar = 0))
...
ui <- navbarPage(
  ...
  header = tags$head(
    ...
    tags$style(HTML("
      ...
      .guide-box {
        background: #EEF7FC; border-left: 4px solid #55A9C8; border-radius: 4px;
  ...
  tabPanel("Overview", br(),
           fluidRow(
             column(width = 8,
                    div(class = "ov-hero-card",
                        ...
                        fluidRow(
                          column(6, div(class = "ov-period-card",
                                        div(class = "ov-period-label", icon("calendar"), " Baseline Period"),
                                        div(class = "ov-period-value", "17 May 2046 \u2013 3 June 2046"))),
                          column(6, div(class = "ov-period-card",
                                        div(class = "ov-period-label", icon("calendar"), " Critical Period"),
                                        div(class = "ov-period-value", "4 June 2046 \u2013 5 June 2046")))
                        )
                    )
             ),
             column(width = 4,
           ...
           br(),
           fluidRow(
             column(3, div(class = "ov-kpi-card kpi-blue",
             ...
             column(3, div(class = "ov-kpi-card kpi-green",
             ...
             column(3, div(class = "ov-kpi-card kpi-purple",
             ...
             column(3, div(class = "ov-kpi-card kpi-red",
           ...
           br(),
           fluidRow(
             column(3, div(class = "ov-roadmap-card",
                           div(class = "ov-roadmap-number blue-dot",   "1"),
                           div(class = "ov-roadmap-title",  "Path to Breach"),
                           div(class = "ov-roadmap-text",   "Reconstruct the sequence of events and communication activity leading to the inappropriate release."),
                           div(class = "ov-roadmap-mini",   "Early signals  \u2022  Escalation  \u2022  Breach"),
                           div(class = "ov-roadmap-link",   "Explore Path to Breach ", icon("arrow-right")))),
             column(3, div(class = "ov-roadmap-card",
                           div(class = "ov-roadmap-number green-dot",  "2"),
                           div(class = "ov-roadmap-title",  "Behaviour Change"),
                           div(class = "ov-roadmap-text",   "Compare agent vocabulary, relationships, and channels before and during the critical period."),
                           div(class = "ov-roadmap-mini",   "Baseline network  vs  Critical network"),
                           div(class = "ov-roadmap-link",   "Explore Behaviour Change ", icon("arrow-right")))),
             column(3, div(class = "ov-roadmap-card",
                           div(class = "ov-roadmap-number purple-dot", "3"),
                           div(class = "ov-roadmap-title",  "Leading Indicators"),
                           div(class = "ov-roadmap-text",   "Identify early warning signals, repeated patterns, and possible oversight gaps before the breach."),
                           div(class = "ov-roadmap-mini",   "Risk signals  \u2022  Similar episodes  \u2022  Gaps"),
                           div(class = "ov-roadmap-link",   "Explore Leading Indicators ", icon("arrow-right")))),
             column(3, div(class = "ov-roadmap-card",
                           div(class = "ov-roadmap-number orange-dot", "4"),
                           div(class = "ov-roadmap-title",  "Evidence Explorer"),
                           div(class = "ov-roadmap-text",   "Search and inspect message-level evidence, risk signals, agents, and communication channels."),
                           div(class = "ov-roadmap-mini",   "Search  \u2022  Filter  \u2022  Validate"),
           ...
           fluidRow(
             column(7,
                    ...
                    fluidRow(
                      column(4, div(class = "ov-highlight-card highlight-blue",
                                    div(class = "ov-highlight-icon",  icon("users")),
                                    div(class = "ov-highlight-title", "Concentration of Communication"),
                                    div(class = "ov-highlight-text",  "Identify agents and channels with elevated communication activity during the critical period."),
                                    div(class = "ov-highlight-link",  "Review agents ", icon("arrow-right")))),
                      column(4, div(class = "ov-highlight-card highlight-red",
                                    div(class = "ov-highlight-icon",  icon("exclamation-circle")),
                                    div(class = "ov-highlight-title", "Elevated Risk Activity"),
                                    div(class = "ov-highlight-text",  "Compare high-risk messages and public or anonymous actions across the investigation windows."),
                                    div(class = "ov-highlight-link",  "Review high-risk messages ", icon("arrow-right")))),
                      column(4, div(class = "ov-highlight-card highlight-green",
                                    div(class = "ov-highlight-icon",  icon("chart-line")),
                                    div(class = "ov-highlight-title", "Early Warning Detected"),
                                    div(class = "ov-highlight-text",  "Examine prior episodes, unusual behaviour, and potential oversight gaps before the breach."),
                                    div(class = "ov-highlight-link",  "Review leading indicators ", icon("arrow-right"))))
                    )
             ),
             column(5,
           ...
           br(),
           div(class = "ov-footer-note", icon("info-circle"),
  ...
  tabPanel(tagList(icon("road"), "1. Path to Breach"), br(),
           card(
             card_header("Question 1: What events and relationships led to the inappropriate release?"),
           ...
           sidebarLayout(
             sidebarPanel(width = 3,
                          div(class = "filter-panel",
                              ...
                              sliderInput("q1_phrase_count",  "Emerging phrases shown", min = 6, max = 20, value = 12)
                          )
             ),
             mainPanel(width = 9,
                       div(class = "guide-box",
                       ...
                       tabsetPanel(
                         tabPanel("1. Interaction Shift", br(),
                                  fluidRow(
                                    column(6, card(card_header("Morning Interaction Structure"),
                                                   div(class="small-note","Before 12:00 PM. Nodes are sized by subgraph centrality."),
                                                   visNetworkOutput("q1_morning_network", height="570px"))),
                                    column(6, card(card_header("Afternoon Interaction Structure"),
                                                   div(class="small-note","From 12:00 PM onward. Compare node size and relationship concentration."),
                                                   visNetworkOutput("q1_afternoon_network", height="570px")))
                                  ),
                                  br(),
                                  fluidRow(
                         ...
                         tabPanel("2. Emerging Phrases", br(),
                                  card(card_header("War-Room Analysis: Emerging Phrases"),
                                       div(class="small-note","Phrases are calculated from the most central afternoon agents and channels/recipients. Only phrases that became more frequent in the afternoon are shown."),
                                       plotlyOutput("q1_phrase_plot", height="480px")),
                                  br(),
                         ...
                         tabPanel("3. Forensic Pivot and Evidence", br(),
                                  ...
                                  fluidRow(
                                    column(6, card(card_header("Vocabulary Continuity"),
                                                   div(class="small-note","Lower values indicate a sharper change in discussion vocabulary."),
                                                   plotlyOutput("q1_similarity_plot", height="310px"))),
                                    column(6, card(card_header("Interaction Cohesion"),
                                                   div(class="small-note","Higher values indicate a stronger shared connection pattern between the two most central afternoon agents."),
                                                   plotlyOutput("q1_adamic_plot", height="310px")))
                                  ),
                                  br(),
                                  card(card_header("Detected Pivot Window"), uiOutput("q1_pivot_note")),
                                  br(),
                                  card(card_header("Focused Message Evidence"),
                                       div(class="small-note","Messages within \u00b130 minutes of the strongest detected pivot, limited to the two most central afternoon agents."),
  ...
  tabPanel(tagList(icon("exchange-alt"), "2. Behaviour Change"), br(),
           card(
             card_header("Question 2: How did communication behaviour during the breach differ from prior behaviour?"),
             p("Compare the distinctive terms used by agents before the breach with those used during the critical period. ",
           ...
           sidebarLayout(
             ...
             mainPanel(width = 9,
                       ...
                       fluidRow(
                         column(6, card(card_header("Baseline Network Structure"),
                                        div(class="small-note","Distinctive agent vocabulary before 4 June 2046."),
                                        visNetworkOutput("baseline_network", height="560px"))),
                         column(6, card(card_header("Critical-Period Network Structure"),
                                        div(class="small-note","Distinctive agent vocabulary during 4\u20135 June 2046."),
                                        visNetworkOutput("critical_network", height="560px")))
                       ),
                       br(),
                       card(card_header("Behaviour Comparison Summary"),
                            div(class="small-note","A direct comparison of communication volume, active agents, channels, and high-risk messages across the two selected periods."),
  ...
  tabPanel(tagList(icon("warning"), "3. Leading Indicators"), br(),
           sidebarLayout(
             sidebarPanel(width = 3,
                          div(class = "filter-panel",
                              h3("Investigation Filters"),
                              dateRangeInput("q3_dates", "Prior period", min_date, baseline_end_default,
                                             min = min_date, max = max_date),
                              selectizeInput("q3_agents",   "Agent / role", c("All", all_agents, all_roles),
                                             selected = "All", multiple = TRUE),
                              selectizeInput("q3_channels", "Channel", all_channels,
                                             selected = all_channels, multiple = TRUE),
                              checkboxGroupInput("q3_risks", "Risk level", all_risks, selected = all_risks)
                          )
             ),
             mainPanel(width = 9,
                       uiOutput("q3_summary"),
                       card(card_header("Prior-Period Activity — by Risk Level"),
                            plotlyOutput("q3_plot", height = "300px")),
                       br(),
                       card(card_header("High Signal Messages by Agent Over Time"),
                            div(class = "small-note",
                                "Shows which agents were sending high-risk messages before the breach, and when they first appeared."),
                            plotlyOutput("q3_agent_plot", height = "260px")),
                       br(),
                       card(card_header("Prior-Period Evidence"),
                            div(class = "small-note",
                                "\u26a0 = embargo terms detected.  \U0001F512 = sent via covert channel (side_huddle / one_on_one_chat)."),
                            DTOutput("q3_table"))
             )
           )
  ),
...
server <- function(input, output, session) {
  ...
  output$q1_morning_agents    <- renderTable({ q1_morning_structure()$agent_ranking   |> slice_head(n=5) },
                                             striped=TRUE, bordered=TRUE, hover=TRUE, spacing="s", rownames=FALSE)
  output$q1_afternoon_agents  <- renderTable({ q1_afternoon_structure()$agent_ranking |> slice_head(n=5) },
                                             striped=TRUE, bordered=TRUE, hover=TRUE, spacing="s", rownames=FALSE)
  q1_phrases <- reactive({ q1_emerging_bigrams(q1_day_data(), q1_afternoon_structure(), input$q1_phrase_count) })
  output$q1_phrase_plot <- renderPlotly({
    phrase_data <- q1_phrases()
    if (nrow(phrase_data) == 0)
      return(q1_empty_plot("No afternoon-emerging phrases match the current filters."))
    plot_ly(phrase_data |> arrange(Shift), x=~Shift, y=~bigram, type="bar", orientation="h",
            marker=list(color="#991B1B"),
            text=~paste0("Phrase: ",bigram,"<br>Morning: ",Morning,"<br>Afternoon: ",Afternoon,"<br>Increase: +",Shift),
            hoverinfo="text") |>
      layout(xaxis=list(title="Frequency increase: Afternoon minus Morning"), yaxis=list(title=""),
             margin=list(l=170,r=30,b=60,t=25), paper_bgcolor="#FFFFFF", plot_bgcolor="#FFFFFF")
  })
  output$q1_phrase_table <- renderDT({
    phrase_data <- q1_phrases()
    if (nrow(phrase_data) == 0)
      return(datatable(data.frame(Message="No afternoon-emerging phrases match the current filters."),
                       rownames=FALSE, options=list(dom="t")))
    datatable(phrase_data, rownames=FALSE,
              options=list(pageLength=10, lengthChange=FALSE, scrollX=TRUE, order=list(list(3,"desc"))))
  })
  ...
  output$q1_similarity_plot <- renderPlotly({
    ...
    if (nrow(similarity_data) == 0)
      return(q1_empty_plot("At least two 30-minute communication windows are required."))
    plot_ly(similarity_data, x=~time_bin, y=~jaccard_similarity, type="scatter", mode="lines+markers",
            ...
            text=~paste0("Time: ",format(time_bin,"%H:%M"),"<br>Jaccard similarity: ",round(jaccard_similarity,3)),
            hoverinfo="text") |>
      layout(xaxis=list(title="Time"), yaxis=list(title="Vocabulary similarity",range=c(0,1)),
             paper_bgcolor="#FFFFFF", plot_bgcolor="#FFFFFF")
  })
  output$q1_adamic_plot <- renderPlotly({
    adamic_data <- q1_adamic_data()
    if (nrow(adamic_data) == 0)
      return(q1_empty_plot("Two central afternoon agents are required to calculate this score."))
    plot_ly(adamic_data, x=~time_bin, y=~adamic_adar, type="scatter", mode="lines+markers",
            ...
            text=~paste0("Time: ",format(time_bin,"%H:%M"),"<br>Shared-connection score: ",round(adamic_adar,3)),
            hoverinfo="text") |>
      layout(xaxis=list(title="Time"), yaxis=list(title="Adamic-Adar-style score"),
             paper_bgcolor="#FFFFFF", plot_bgcolor="#FFFFFF")
  })
  q1_pivot_time <- reactive({
    vocabulary_data <- q1_vocabulary_data(); adamic_data <- q1_adamic_data()
    text_pivot <- if (nrow(vocabulary_data) > 0)
      vocabulary_data |> slice_min(jaccard_similarity,n=1,with_ties=FALSE) |> pull(time_bin)
    else as.POSIXct(NA)
    connection_peak <- if (nrow(adamic_data) > 0 && any(adamic_data$adamic_adar > 0))
      adamic_data |> slice_max(adamic_adar,n=1,with_ties=FALSE) |> pull(time_bin)
    else as.POSIXct(NA)
    list(text_pivot=text_pivot, connection_peak=connection_peak,
  ...
  output$q1_pivot_note <- renderUI({
    ...
    if (is.na(pivot$anchor))
      return(div(class="small-note","No pivot window can be identified with the current filters."))
    tagList(
      tags$p(tags$b("Primary evidence window: "), format(pivot$anchor, "%d %B %Y, %H:%M")),
  ...
  q1_focused_evidence <- reactive({
    ...
    if (is.na(pivot$anchor) || length(actors) == 0) return(data[0,])
    data |> filter(q1_event_time >= pivot$anchor - lubridate::minutes(30),
                   q1_event_time <= pivot$anchor + lubridate::minutes(30),
                   agent_label %in% actors) |> arrange(q1_event_time)
  })
  output$q1_evidence_table <- renderDT({
    evidence <- q1_focused_evidence()
    if (nrow(evidence) == 0)
      return(datatable(data.frame(Message="No messages were found in the detected evidence window."),
                       rownames=FALSE, options=list(dom="t")))
    datatable(
      evidence |> transmute(
        Time=format(q1_event_time,"%Y-%m-%d %H:%M"), Agent=agent_label, Role=agent_role,
        Channel=channel, `Risk signal`=risk_signal_group,
        `Internal state`=internal_state_deliberating,
        `Message evidence`=if_else(nchar(content)>300,paste0(str_sub(content,1,300),"\u2026"),content)),
  ...
  output$baseline_network <- renderVisNetwork({
    draw_alvin_keyword_network(build_alvin_keyword_network(baseline_data(), input$keywords_per_agent))
  })
  output$critical_network <- renderVisNetwork({
    draw_alvin_keyword_network(build_alvin_keyword_network(critical_data(), input$keywords_per_agent))
  })
  output$q2_summary <- renderTable({
    b <- baseline_data() |> summarise(Messages=n(), `Active agents`=n_distinct(agent_id),
                                      `Active channels`=n_distinct(channel),
                                      `High-risk messages`=sum(risk_signal_group=="High Signal",na.rm=TRUE))
    cr <- critical_data() |> summarise(Messages=n(), `Active agents`=n_distinct(agent_id),
                                       `Active channels`=n_distinct(channel),
                                       `High-risk messages`=sum(risk_signal_group=="High Signal",na.rm=TRUE))
    mn <- names(b); bv <- as.numeric(b[1,]); cv2 <- as.numeric(cr[1,]); dv <- cv2-bv
    change_labels <- ifelse(bv==0, ifelse(cv2==0,"No change","New in critical period"),
                            paste0(ifelse(dv>0,"+",""),dv," (",
                                   ifelse(round(dv/bv*100,1)>0,"+",""),round(dv/bv*100,1),"%)"))
    tibble(Metric=mn, Baseline=bv, `Critical period`=cv2, Change=change_labels)
  }, striped=TRUE, bordered=TRUE, hover=TRUE, spacing="s", rownames=FALSE)
  ...
  output$q3_summary <- renderUI({
    d <- q3_data()
    if (nrow(d) == 0) return(NULL)
    n_high    <- sum(d$risk_signal_group == "High Signal", na.rm = TRUE)
    n_covert  <- sum(d$channel %in% c("side_huddle","one_on_one_chat"), na.rm = TRUE)
    n_embargo <- sum(d$contains_embargo_terms == TRUE, na.rm = TRUE)
    high_dates <- d$event_date[d$risk_signal_group == "High Signal"]
    first_high <- if (length(high_dates) > 0)
      format(min(high_dates, na.rm = TRUE), "%d %b %Y") else "none"
    div(class = "guide-box",
        tags$b(n_high), " High Signal messages in this period ",
        "(first appeared ", tags$b(first_high), ").  ",
        tags$b(n_covert), " messages sent via covert channels.  ",
        tags$b(n_embargo), " messages contained embargo-related terms.")
  })
  
  output$q3_plot <- renderPlotly({
    d <- q3_data()
    validate(need(nrow(d) > 0, "No messages match the selected filters."))
    plot_data <- d |>
      count(event_date, risk_signal_group) |>
      mutate(risk_signal_group = factor(risk_signal_group,
                                        levels = c("High Signal","Moderate Signal","Low Signal","No Signal")))
    plot_ly(plot_data, x=~event_date, y=~n, type="bar", color=~risk_signal_group,
            colors = c("High Signal"="#C00000","Moderate Signal"="#E8820C",
                       "Low Signal"="#2E75B6","No Signal"="#E0E0E0")) |>
  ...
  output$q3_agent_plot <- renderPlotly({
    d <- q3_data() |> filter(risk_signal_group == "High Signal") |>
      count(event_date, agent_label)
    validate(need(nrow(d) > 0,
                  "No High Signal messages in the selected period. Try widening the date range."))
    plot_ly(d, x=~event_date, y=~n, color=~agent_label, type="scatter", mode="lines+markers",
            marker=list(size=8),
            hovertemplate=paste0("<b>%{x}</b><br>%{customdata}<br>High Signal msgs: %{y}<extra></extra>"),
            customdata=~agent_label) |>
      layout(xaxis=list(title="Date"),
             yaxis=list(title="High Signal messages", dtick=1),
  ...
  output$q3_table <- renderDT({
    ...
    d |>
      transmute(
        ...
        Channel          = channel,
        `Risk signal`    = risk_signal_group,
        `Near breach`    = replace_na(as.character(near_breach_window), "\u2014"),
        `Embargo terms`  = ifelse(contains_embargo_terms == TRUE, "\u26a0 Yes", ""),
        `Covert channel` = ifelse(channel %in% c("side_huddle","one_on_one_chat"),
                                  "\U0001F512 Yes", ""),
        `Message evidence` = if_else(nchar(content) > 200,
                                     paste0(str_sub(content,1,200),"\u2026"), content)
      ) |>
  ...
  # ── Q4 ────────────────────────────────────────────────────────────────────────
  q4_data <- reactive({
    data <- filter_messages(communications, input$q4_dates, input$q4_agents,
                            input$q4_channels, input$q4_risks)
  ...
  output$q4_table <- renderDT({
    datatable(message_table(q4_data()), rownames=FALSE, filter="top",
